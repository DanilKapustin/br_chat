...
################################
FROM python:3.10-slim AS cpu_base
...
# Sets up all our shared environment variables
################################
FROM nvidia/cuda:12.2.2-cudnn8-devel-ubuntu22.04 AS gpu_base
...
################################
FROM cpu_base AS cpu-app-builder
...
# Used to build deps + create our virtual environment
################################
FROM gpu_base AS gpu-app-builder
...
################################
# API
# API runtime - we don't need GPU here
################################
FROM cpu_base AS api
...
COPY --from=cpu-app-builder $PYSETUP_PATH $PYSETUP_PATH
COPY . /app/
...
# Background runtime
################################
FROM gpu_base AS background

# Environment variables
ENV E5_MODEL_PATH=/app/model/e5
ENV BACKGROUND_WORKERS=4

COPY --from=gpu-app-builder $PYSETUP_PATH $PYSETUP_PATH
COPY . /app/
